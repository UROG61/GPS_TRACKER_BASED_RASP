...
def start_gps_receiver(serial, dmesg):    # Gps in alıcısının başlayıp başlamadıgını kontrol ediyor ve başlıyor. 
    t2 = threading.Thread(target=get_gps_data, args=(serial, dmesg))  # thread fonksiyonu sayesinde eşzamanlı çalışılabiliyor, target sürekli çalışıyor ve hedefi üstteki def fonksiyonu , "args" dışarıdan bir değişken taşımak için
    t2.start()    # 
